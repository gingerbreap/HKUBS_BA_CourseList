...
/** Sum of localStorage keys owned by this app (`msba-*`). */
export function measureUserDataBytes(): number {
  let total = 0
  try {
    for (let i = 0; i < localStorage.length; i += 1) {
      const key = localStorage.key(i)
      if (!key || !key.startsWith('msba-')) continue
...
/** Remove all `msba-*` localStorage keys, then reload. */
export function resetUserDataAndReload(): void {
  try {
    const keys: string[] = []
    for (let i = 0; i < localStorage.length; i += 1) {
      const key = localStorage.key(i)
      if (key?.startsWith('msba-')) keys.push(key)
    }
